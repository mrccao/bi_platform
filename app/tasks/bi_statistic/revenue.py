...
def process_bi_statistic_revenue(target):
    ...
    def collection_revenue_every_platform(connection, transaction):
    ...
    result_proxy = with_db_context(db, collection_revenue_every_platform)
    ...
    if rows:
        ...
        def sync_collection_revenue_every_platform(connection, transaction):
            ...
            try:
                ...
                print(target + ' revenue for every platform transaction.rollback()')
                transaction.rollback()
                raise
            else:
                transaction.commit()
                print(target + ' revenue for every platform transaction.commit()')

        with_db_context(db, sync_collection_revenue_every_platform)
